...
from backend.data.mongodb import documents_collection
...
@app.post("/tutor-chat")
def tutor_chat(req: ConversationRequest):
...
def interview_chat(req: ConversationRequest):
...
@app.post("/interview")
def interview(req: TopicRequest):
...
def rag_test(req: RAGRequest):
    ...
    context, citations = pipeline.search(req.question)
    prompt = f"""
    You are an AI study assistant.
